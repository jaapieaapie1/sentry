...
interface Props {
  ...
  /**
   * Force the stale status for testing purposes. When enabled, disables the
   * API query and sets the forced status.
   */
  force?: 'fresh' | 'stale';
}
...
export function FrontendVersionProvider({children, force, releaseVersion}: Props) {
  ...
  const forcedStaleness = {
    stale: true,
    fresh: false,
  };

  const stale = force
    ? forcedStaleness[force]
    : !!deployedVersion && deployedVersion !== runningVersion;
